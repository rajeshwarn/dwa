	...
	// Button Open Request Movie
	$(" .btn-open-request-movie ").click(function(){

		var mov_id = $(this).attr('data-id');
		var request_title = $(this).parent().parent().find(".request-title").text();

		$(" #modal-view-request-movie ").modal();
		$(" #modal-view-request-movie .modal-body ").html("<p>" + request_title + "</p>");
		...
		$.ajax({
			type: 'GET',
			data: '',
			url: 'http://dewabioskop21.com/dev-admin/open-request-movie/'+mov_id,
		...
		else
			{
				$.ajax({
					...
					success: function(obj) {
						...
						$(" #dashboard-admin-section .right-dashboard .coming-soon-movie-wrap ul ").append("<li><span class='movie-title'>" + mov_title + "</span><a href='delete-coming-soon-movie/" + mov_id + "' class='btn-trash-coming-soon-movie'><i class='fa fa-trash' aria-hidden='true'></i></a></li>")
					},
					error: function (xhr, ajaxOptions, thrownError) {
						console.log(xhr.status);
						console.log(thrownError);
